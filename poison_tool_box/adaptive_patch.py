...
class poison_generator():
    ...
    def __init__(self, img_size, dataset, poison_rate, 
                 path, trigger_names, alphas, 
                 target_class=0, cover_rate=0.01,
                 random_seed=0,num_classes=10
                 ):
            ...
            if pt < num_poison and poison_indices[pt] == i:
                ...
                for j in range(k):
                    if pt < (j + 1) * (num_poison / k):
                        # print(img.shape)
                        # print(self.trigger_masks[j].shape)
                        img = img + self.alphas[j] * self.trigger_masks[j] * (self.trigger_marks[j] - img)
                        # img[j, :, :] = img[j, :, :] + self.alphas[j] * self.trigger_masks[j] * (self.trigger_marks[j][j, :, :] - img[j, :, :])
                        break
                pt += 1
...
class poison_transform():
    ...
    def __init__(self, img_size, test_trigger_names, test_alphas, target_class=0, denormalizer=None, normalizer=None, device='cpu'):
        ...
        for i in range(len(test_trigger_names)):
            ...
            self.trigger_marks.append(trigger.to( self.device))
            self.trigger_masks.append(trigger_mask.to( self.device))
